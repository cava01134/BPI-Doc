...
void loop()
{
 ...
 輸入完成後，點擊“編譯”檢查代碼有無錯誤。確保沒有錯誤後就可以開始上傳了，點擊“上傳”之後 IDE  會把代碼發送給 Leaf-S3 主板。打開 Arduino 串口監視器，可以看到打印信息如下： 

![](../assets/images/Lesson5-2.png)

### SPI實驗：代碼分析

 本項目採用的是 BME280 庫文件，在 Item-5.ino 文件中並沒有對 SPI 底層進行操作，不過，Leaf  ESP32-S3 的 SPI 使用完全兼容 Arduino。
